...
const AdminLayout = () => {
  ...
  const menuItems = [
    ...
    { path: "/admin/chat-config", label: t("chatbox.admin_menu"), icon: Bot },
  ];
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex relative overflow-x-hidden">
      ...
      <aside
        className={`w-64 text-white flex flex-col fixed h-full z-50 transition-transform duration-300 transform ${
      ...
      >
        ...
        <nav className="flex-1 p-4 space-y-1 overflow-y-auto" data-lenis-prevent>
          {menuItems.map((item) => {
            ...
            const isChatConfig = item.path === "/admin/chat-config";

            return (
              <div key={item.path}>
                {isChatConfig && <div className="border-t border-white/10 my-2" />}
                <NavLink
                  to={item.path}
                  onClick={() => setIsSidebarOpen(false)}
                  className={({ isActive }) =>
